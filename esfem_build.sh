...
Usage:
  bash esfem_build.sh
  bash esfem_build.sh --rebuild
  bash esfem_build.sh --build-clean
  bash esfem_build.sh --clean
  bash esfem_build.sh --clean-all
  bash esfem_build.sh --ensure

Options:
  --rebuild    Remove build before rebuilding the ES-FEM executable
  --build-clean Remove build and bin before rebuilding the ES-FEM executable
  --clean      Remove build and the ES-FEM binary
...
needs_rebuild() {
    ...
    if [[ "${ROOT_DIR}/CMakeLists.txt" -nt "${BIN_FILE}" ||
          ...
          "${ROOT_DIR}/esfem_build.sh" -nt "${BIN_FILE}" ]]; then
        return 0
    fi
...
build_project() {
    ...
    cmake --build "${BUILD_DIR}" --target esfem -j
}
