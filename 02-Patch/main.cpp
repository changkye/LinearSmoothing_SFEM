...
namespace
{
    ...
    void print_help()
    {
        ...
        std::cout
            << "Usage:\n"
            << "  " << exe_name << " [linear_patch|high_order_patch|mode_test] [options]\n\n"
            << "Options:\n"
            << "  --num-els <n|nxm>\n"
            << "  --num-els-x <n>\n"
            << "  --num-els-y <m>\n"
...
int main(int argc, char **argv)
{
    try
    {
        ...
        for (const Method method : methods)
        {
            ...
            {
                const int num_negative = static_cast<int>(result.relative_error);
                const int n_report = std::min(static_cast<int>(result.eigenvalues.size()), 12);
                std::cout << std::scientific << std::setprecision(4);
                std::cout << scenario_name(scenario) << " / " << method_name(method)
                          << ": num_dof=" << result.eigenvalues.size()
                          << "  zero_modes=" << result.num_zero_modes
                          << " (expected 3)"
                          << "  negative=" << num_negative << '\n';
                std::cout << "  First " << n_report << " eigenvalues:\n";
                for (int i = 0; i < n_report; ++i)
                {
                    std::cout << "    lambda[" << std::setw(2) << i + 1 << "] = "
                              << result.eigenvalues[static_cast<std::size_t>(i)] << '\n';
                }
                std::cout << "  Min positive eigenvalue = " << result.strain_energy << '\n';
                if (num_negative > 0)
                {
                    std::cout << "  WARN: K is not positive semi-definite ("
                              << num_negative << " negative eigenvalue(s)).\n";
                }
                if (result.num_zero_modes == 3 && num_negative == 0)
                {
                    std::cout << "  PASS: exactly 3 rigid-body modes, no spurious zero-energy modes.\n";
                }
                else if (num_negative == 0)
                {
                    std::cout << "  FAIL: " << (result.num_zero_modes - 3)
                              << " spurious zero-energy mode(s) detected.\n";
                }

                // Write eigenvalue summary file
                std::filesystem::create_directories(run_dir);
                const std::filesystem::path eig_file = run_dir / (base_name + "_eigenvalues.txt");
                std::ofstream eig_out(eig_file);
                eig_out << std::scientific << std::setprecision(16);
                eig_out << "num_dof " << result.eigenvalues.size() << '\n';
                eig_out << "num_zero_modes " << result.num_zero_modes << '\n';
                eig_out << "num_negative_modes " << num_negative << '\n';
                eig_out << "min_positive_eigenvalue " << result.strain_energy << '\n';
                for (std::size_t i = 0; i < result.eigenvalues.size(); ++i)
                {
                    eig_out << "lambda[" << i + 1 << "] " << result.eigenvalues[i] << '\n';
                }
            }
            else
            {
