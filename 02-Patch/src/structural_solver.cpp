...
namespace sfem
{
    namespace
    {
        ...
        {
            return false; // 02-Patch: linear elasticity only
        }
...
#pragma omp critical(structural_linear_esfem_scatter)
    ...
    // Zero-energy mode test: assemble K without BCs, compute eigenvalues, report
    // how many are at/near zero (rigid body modes = 3 for 2D; spurious if more).
    Result solve_mode_test(const Model &model)
    {
        const int ndof = 2 * model.mesh.nodes.rows();
        std::vector<Triplet> triplets;
        ...
        if (model.method == Method::CSFEM)
        {
            const std::vector<SupportData> cell_supports = build_all_cell_support_data(model);
            for (int ie = 0; ie < static_cast<int>(cell_supports.size()); ++ie)
            {
                const SupportData &data = cell_supports[static_cast<std::size_t>(ie)];
                Eigen::MatrixXd ke = Eigen::MatrixXd::Zero(12, 12);
        ...
        {
            const std::vector<SupportData> edge_supports = build_all_edge_support_data(model, false);
            int neg_weight_edges = 0;
            int neg_weight_total = 0;
            for (int ie = 0; ie < static_cast<int>(edge_supports.size()); ++ie)
            {
                const SupportData &data = edge_supports[static_cast<std::size_t>(ie)];
                bool edge_has_neg = false;
                for (int ig = 0; ig < static_cast<int>(data.weights.size()); ++ig)
                {
                    if (data.weights[static_cast<std::size_t>(ig)] < 0.0)
                    {
                        ++neg_weight_total;
                        edge_has_neg = true;
                    }
                }
                if (edge_has_neg) { ++neg_weight_edges; }
            ...
            // Check if any individual ke has negative eigenvalues
            int ke_neg_edges = 0;
            double ke_min_eval = 0.0;
            for (int ie2 = 0; ie2 < static_cast<int>(edge_supports.size()); ++ie2)
            {
                const SupportData &d2 = edge_supports[static_cast<std::size_t>(ie2)];
                Eigen::MatrixXd ke2 = Eigen::MatrixXd::Zero(d2.edof.size(), d2.edof.size());
                for (int ig = 0; ig < d2.points.rows(); ++ig)
                {
                    Eigen::MatrixXd b2 = Eigen::MatrixXd::Zero(3, d2.edof.size());
                    for (int a = 0; a < static_cast<int>(d2.nodes.size()); ++a)
                    {
                        b2(0, 2 * a)     = d2.dx(ig, a);
                        b2(1, 2 * a + 1) = d2.dy(ig, a);
                        b2(2, 2 * a)     = d2.dy(ig, a);
                        b2(2, 2 * a + 1) = d2.dx(ig, a);
                    }
                    ke2 += b2.transpose() * model.cmat * b2 * d2.weights[static_cast<std::size_t>(ig)];
                }
                const Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> ke_eigs(ke2);
                const double min_ev = ke_eigs.eigenvalues().minCoeff();
                ke_min_eval = std::min(ke_min_eval, min_ev);
                if (min_ev < -1.0e-10)
                {
                    ++ke_neg_edges;
                    std::cerr << "  [bad ke] ie=" << ie2
                              << "  nn=" << d2.nodes.size()
                              << "  nig=" << d2.weights.size()
                              << "  max|dx|=" << d2.dx.cwiseAbs().maxCoeff()
                              << "  max|dy|=" << d2.dy.cwiseAbs().maxCoeff()
                              << "  min_ev=" << min_ev << '\n';
                    std::cerr << "    weights:";
                    for (double w : d2.weights) { std::cerr << ' ' << w; }
                    std::cerr << '\n';
                    std::cerr << "    gcoord:\n" << d2.gcoord << '\n';
                }
            }
            std::cerr << "[ESFEM mode_test] total_edges=" << edge_supports.size()
                      << "  edges_with_neg_weight=" << neg_weight_edges
                      << "  neg_weights_total=" << neg_weight_total
                      << "  ke_neg_edges=" << ke_neg_edges
                      << "  ke_min_eigenvalue=" << ke_min_eval << '\n';
        }
        else // FEM
        {
            const QuadratureRule qr = triangle_rule(2);
        ...
        // Dense eigenvalue decomposition (no BCs applied)
        const SparseMatrix k_sparse = make_sparse_matrix(ndof, ndof, triplets);
        const Eigen::MatrixXd k_dense = Eigen::MatrixXd(k_sparse);
        const Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> eigs(k_dense);
        const Eigen::VectorXd &evals = eigs.eigenvalues();

        // Threshold based on the MEDIAN of |λ|: robust against outlier
        // large eigenvalues (e.g., ill-conditioned ESFEM without BCs).
        std::vector<double> abs_evals(static_cast<std::size_t>(evals.size()));
        for (int i = 0; i < evals.size(); ++i)
        {
            abs_evals[static_cast<std::size_t>(i)] = std::abs(evals(i));
        }
        std::sort(abs_evals.begin(), abs_evals.end());
        const double median_abs = abs_evals[abs_evals.size() / 2];
        constexpr double kZeroTol = 1.0e-8;
        const double threshold = kZeroTol * std::max(median_abs, 1.0e-30);

        Result result;
        result.eigenvalues.resize(static_cast<std::size_t>(evals.size()));
        for (int i = 0; i < evals.size(); ++i)
        {
            result.eigenvalues[static_cast<std::size_t>(i)] = evals(i);
        }
        // Count: (a) near-zero |λ| ≤ threshold,
        //        (b) negative  λ  < −threshold  (non-PSD indicator)
        result.num_zero_modes = 0;
        int num_negative = 0;
        for (int i = 0; i < evals.size(); ++i)
        {
            if (std::abs(evals(i)) <= threshold)
            {
                ++result.num_zero_modes;
            }
            else if (evals(i) < -threshold)
            {
                ++num_negative;
            }
        }
        // Store min positive eigenvalue in strain_energy for convenience
        result.strain_energy = std::numeric_limits<double>::quiet_NaN();
        for (int i = 0; i < evals.size(); ++i)
        {
            if (evals(i) > threshold)
            {
                result.strain_energy = evals(i);
                break;
            }
        }
        // Pack num_negative into relative_error field for reporting
        result.relative_error = static_cast<double>(num_negative);
        result.nodal_u = Eigen::MatrixXd::Zero(model.mesh.nodes.rows(), 2);
        return result;
    }
...
#pragma omp parallel for if (edge_supports.size() > 1) default(none) shared(model, uu, triplet_buffers, residual_buffers, nthreads, edge_supports) reduction(+ : energy)
    ...
    {
        if (problem.scenario() == Scenario::ModeTest)
        {
            return solve_mode_test(problem.data());
        }
