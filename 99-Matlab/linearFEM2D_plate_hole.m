...
if model.flag == 1
	...
	if model.txtFlag == 1
	    ...
	    for iel = 1:size(model.Elements,1)
	    	text(mean(newNodes(model.Elements(iel,:),1)),...
	    		mean(newNodes(model.Elements(iel,:),2)),num2str(iel),'color','r');
	    end
	end
	bcTitle = 'Plate with a hole';
	title(bcTitle);
	saveas(gcf,[resultpath '/LinearFEM2D_' model.elemType '_' model.bcType '_' probType],'epsc2');
	hold off
end
