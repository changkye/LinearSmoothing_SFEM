function model = linearSFEM2D_cell_stiffness_T6_1(model)
    ...
    [Wi,Qi] = quadrature(3,'TRIANGULAR',2);
    ...
    for ivo = 1:size(model.Elements,1)
        ...
        xy = gcoord;
        Ni = zeros(5,size(wkX,1));
        Wmat = zeros(6,size(Wi,1));
        for ig = 1:size(Wi,1)
            ...
            Ni(2,:) = Ni(2,:) + 2*(N*Wi(ig)*detJ0(ig)*mR(ig,1))';
            Ni(3,:) = Ni(3,:) + 2*(N*Wi(ig)*detJ0(ig)*mR(ig,2))';
        ...
        fx = zeros(6,size(wkX,1)); fy = zeros(6,size(wkX,1));
        for is = 1:size(bound,1)
            bxy = zeros(6,size(wkX,1));
