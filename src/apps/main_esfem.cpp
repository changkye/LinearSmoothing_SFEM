...
void print_help()
{
    std::cout
        << "Usage:\n"
        << "  ./nonlinear_esfem_t6 [nonlinear_patch] [options]\n\n"
        << "Options:\n"
        << "  --num-els <n>\n"
        << "  --mu <value>\n"
        << "  --lambda_like <value>\n"
...
std::string esfem_base_name(const std::string &problem_type, int num_els_x, int num_els_y)
{
    return "ESFEM_" + problem_type + "_" + std::to_string(num_els_x) + "x" + std::to_string(num_els_y);
}
                            ...
                            const OutputOptions &output_options)
{
    ...
    if (has_exact)
    {
        const std::filesystem::path csv_file = output_dir / (base_name + "_comparison.csv");
        std::ofstream csv(csv_file);
        if (!csv)
        {
            throw std::runtime_error("Failed to open comparison csv: " + csv_file.string());
        }
        csv << std::setprecision(16);
        csv << "node,ux,uy,ux_exact,uy_exact,ux_error,uy_error\n";
        for (int i = 0; i < result.nodal_u.rows(); ++i)
        {
            const double ux = result.nodal_u(i, 0);
            const double uy = result.nodal_u(i, 1);
            const double ux_exact = result.exact_u(i, 0);
            const double uy_exact = result.exact_u(i, 1);
            csv << i << ','
                << ux << ','
                << uy << ','
                << ux_exact << ','
...
int main(int argc, char **argv)
{
    try
    {
        ...
        options.linear_solver = parse_solver_type(model_parameters::default_esfem_linear_solver);
        options.iterative_maxiter = model_parameters::default_esfem_iterative_maxiter;
        options.iterative_tolerance = model_parameters::default_esfem_iterative_tolerance;
        options.compute_exact_solution = model_parameters::default_compute_exact_solution;
        std::filesystem::path output_dir = std::filesystem::path("res/nonlinear_patch/esfem");
        ...
        const nonlinear_esfem_t6::NonlinearEsfemT6Solver solver;
        const auto solve_start = std::chrono::steady_clock::now();
        const nonlinear_esfem_t6::Result result = solver.solve(problem, options);
        const auto solve_end = std::chrono::steady_clock::now();

        const std::string base_name = esfem_base_name("nonlinear_patch", num_els, num_els);
        ...
        write_comparison_files(output_dir, base_name, problem.data().mesh, result, output_options);
        ...
        std::cout << std::setprecision(6)
                  << "nonlinear_patch / esfem: rel_error=" << result.relative_error
