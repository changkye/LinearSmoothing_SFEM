...
namespace nonlinear_esfem_t6::shared
{
    namespace
    {
        ...
        int local_edge_index_from_corner_nodes(const std::array<int, 6> &element,
                                               int node_a, int node_b)
        {
            for (int edge = 0; edge < 3; ++edge)
            {
                ...
                if ((edge_a == node_a && edge_b == node_b) ||
                    (edge_a == node_b && edge_b == node_a))
                {
                    return edge;
                }
            }
    ...
    std::vector<SupportDomain> EdgeSmoothedDomainAssembler::build_domains(
        const Problem &problem) const
    {
        ...
        for (int ie = 0; ie < static_cast<int>(problem.mesh.elements.size()); ++ie)
        {
            ...
            for (int j = 0; j < 3; ++j)
            {
                ...
                for (auto &edge : edges)
                {
                    if ((edge.nodes[0] == n1 && edge.nodes[1] == n2) ||
                        (edge.nodes[0] == n2 && edge.nodes[1] == n1))
                    {
                        edge.elem2 = ie;
                        edge.local_edge2 = j;
                        found = true;
        ...
        for (int ie = 0; ie < static_cast<int>(problem.mesh.elements.size()); ++ie)
        {
            tri_area[static_cast<std::size_t>(ie)] =
                GeometryUtils::triangle_area(GeometryUtils::element_coordinates(
                                                 problem.mesh, problem.mesh.elements[static_cast<std::size_t>(ie)])
                                                 .topRows(3));
        }
