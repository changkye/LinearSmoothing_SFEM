...
model = param('T6',[3e7 0.25],numEls);
model.bcType = bcType;
model.stressType = 'plane_stn';
...
% if strcmp(model.elemType,'T3')
%     result = SFEM2D_edge_T3_cantilever_analytic(model,result);
% else
%     result = SFEM2D_edge_T6_cantilever_analytic_1(model,result);
% end
...
if model.flag == 1
    ...
    if model.txtFlag == 1
        for in = 1:size(newNodes,1)
            xc = newNodes(in,1) + 1e-3;
            yc = newNodes(in,2) - 3e-3;
            text(xc,yc,num2str(in),'color','b');
        end
        for iel = 1:size(model.Elements,1)
    ...
    bcTitle = 'Cantilever';
    title(bcTitle);
    saveas(gcf,[resultpath '/LinearSFEM2D_edge_' model.elemType '_' model.bcType '_' ...
        probType],'epsc2');
    hold off
end
result.L2norm
