function [Nodes,Elements] = elementT4(L,numEls)
	% 
	xn = numEls(1) + 1;
	yn = numEls(2) + 1;
	...
	Elements = [v1 v3 v2; v3 v4 v2];

	% addition of nodes at the centre of each element for the bubble
	[numelem,~] = size(Elements);
	[numnode,~] = size(Nodes);
	
	% additional nodes
	for ie = 1:numelem
		Nodes(numnode+ie,:) = mean(Nodes(Elements(ie,:),:));
	end
	
	% element connectivities for the added nodes
	for ie = 1:numelem
		Elements(ie,4) = numnode + ie;
	end
end