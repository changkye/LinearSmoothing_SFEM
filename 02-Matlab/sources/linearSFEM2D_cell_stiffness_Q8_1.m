function model = linearSFEM2D_cell_stiffness_Q8_1(model)
    ...
    for ivo = 1:size(model.Elements,1)
        ...
        xy = gcoord;
        Ni = zeros(8,size(wkX,1));
        Wmat = zeros(8,size(Wi,1));
        for ig = 1:size(Wi,1)
            ...
            Ni(2,:) = Ni(2,:) + (N*Wi(ig)*detJ0(ig)*mR(ig,1)^2)';
            Ni(3,:) = Ni(3,:) + (N*Wi(ig)*detJ0(ig)*mR(ig,2)^2)';
            Ni(4,:) = Ni(4,:) + (N*Wi(ig)*detJ0(ig)*mR(ig,1))';
            Ni(5,:) = Ni(5,:) + (N*Wi(ig)*detJ0(ig)*mR(ig,2))';
            Ni(6,:) = Ni(6,:) + 2*(N*Wi(ig)*detJ0(ig)*mR(ig,1))';
            Ni(7,:) = Ni(7,:) + 2*(N*Wi(ig)*detJ0(ig)*mR(ig,2))';
            Ni(8,:) = Ni(8,:) + 2*(N*Wi(ig)*detJ0(ig)*mR(ig,1)*mR(ig,2))';
            % compute W matrix
            Wmat(:,ig) = Wi(ig)*detJ0(ig)*[1; mR(ig,1); mR(ig,2);
                mR(ig,1)^2; mR(ig,1)*mR(ig,2); mR(ig,2)^2;
                mR(ig,1)^2*mR(ig,2); mR(ig,1)*mR(ig,2)^2];
        end
        ...
        fx = zeros(8,size(wkX,1)); fy = zeros(8,size(wkX,1));
        for is = 1:size(bound,1)
            bxy = zeros(8,size(wkX,1));
            ...
            for ig = 1:ng
                ...
                xieta = cal_xieta('Q8',xieta_gp,wkX);
                N_T = lagrange_basis('Q8',xieta);
                
                bxy = bxy + [N_T'*detJ*Wb(ig); N_T'*detJ*Wb(ig)*xieta_gp(1); 
                        N_T'*detJ*Wb(ig)*xieta_gp(2); 
                        N_T'*detJ*Wb(ig)*xieta_gp(1)^2;
