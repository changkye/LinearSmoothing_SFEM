function model = linearSFEM2D_edge_stiffness_T6_3(model)
    ...
    for ivo = 1:size(targetEdge,1)
        ...
        if nc == 1
            ...
            node_sc = [];
            node_sc = [1 2 4 5 9 8; 2 3 4 6 7 9];
            for jc = 1:size(node_sc,1)
                idx = node_sc(jc,:);
                % subcell boundary length
                side = cal_side(wkX(idx(1:3),1),wkX(idx(1:3),2));

                % outward normal vectors
                [nx,ny] = cal_nx_ny(wkX(idx(1:3),1),wkX(idx(1:3),2),side);

                % initialise
                fx = zeros(4,size(wkX(idx,:),1)); 
                fy = zeros(4,size(wkX(idx,:),1));
                
                bound = [1 2 4; 2 3 5; 3 1 6];

                % loop over subcell boundary
                for is = 1:size(bound,1)
                    bxy = zeros(4,size(wkX(idx,:),1));
                    X = wkX(idx(bound(is,:)),:);
                    ...
                    for ig = 1:ng
                        ...
                        % shape functions on boundary
                        N_T = zeros(size(wkX,1),1);
                        N_T = getSerendipityShapeFunc_lagrange('T6',wkX(idx,:),xieta_gp);
                ...
                nsf = length(model.supp{neighbour(jc)});
                if jc == 1
                    nn = nsf;
                    Fx = fx;
                    Fy = fy;
                    ...
                    i0 = 0;
                    for jj = 1:nsf
                        nod = model.supp{neighbour(jc)}(jj);
                        flag = 0;
                        for j = 1:nn
                            if nodL(j) == nod
                                Fx(:,j) = Fx(:,j) + fx(:,jj);
            ...
            clear detJ0 mR side nx ny nodL wkX X side nx ny wkInd N
        end
    end
