% nonlinear edge-based smoothed finite element method
% patch test
% neo-Hookean model
...
model.edgeBasisOrder = 2;
model.edgeBoundaryNg = 3;
model.edgeTriQuadOrder = 4;
model.edgeQuadQuadOrder = 3;
model.edgeRegParam = 1e-10;
...
for istp = 1:nstep
    ...
    while (condition>Tolerance) && (niter<maxiter)
        ...
        % Global tangent stiffness & internal force (edge-based)
        model = callNonlinearEdgeStiffness(model,uu);
...
if model.flag == 1
    ...
    title(bcTitle);
    saveas(gcf,[resultpath '/NonlinearSFEM2D_edge_' model.elemType '_' model.bcType '_' ...
...
if strcmp(model.bcType,'PlateHole') && exist('bcCase','var')
    vtk = [resultpath '/NonlinearSFEM2D_edge_' model.elemType '_' model.bcType '_' bcCase '.vtu'];
    save([resultpath '/NonlinearSFEM2D_edge_' model.elemType '_' model.bcType '_' bcCase '.mat'],'model','result');
else
    vtk = [resultpath '/NonlinearSFEM2D_edge_' model.elemType '_' model.bcType '_' ...
        num2str(model.numEls(1)) 'x' num2str(model.numEls(2)) '.vtu'];
    save([resultpath '/NonlinearSFEM2D_edge_' model.elemType '_' model.bcType '_' ...
        num2str(model.numEls(1)) 'x' num2str(model.numEls(2)) '.mat'],'model','result');
end
VTKPostProcess(vtk,model,result);

function model = callNonlinearEdgeStiffness(model,uu)
    if strcmp(model.elemType,'T3') && exist('nonlinearSFEM2D_edge_stiffness_T3','file')
        model = nonlinearSFEM2D_edge_stiffness_T3(model,uu);
    elseif strcmp(model.elemType,'T6') && exist('nonlinearSFEM2D_edge_stiffness_T6','file')
        model = nonlinearSFEM2D_edge_stiffness_T6(model,uu);
    elseif exist('nonlinearSFEM2D_edge_stiffness','file')
        model = nonlinearSFEM2D_edge_stiffness(model,uu);
    else
        error('patch_nonlinearSFEM2D_edge:MissingEdgeStiffness', ...
            ['Edge-based nonlinear stiffness function not found. Add one of: ' ...
             'nonlinearSFEM2D_edge_stiffness_T3.m, nonlinearSFEM2D_edge_stiffness_T6.m, ' ...
             'or nonlinearSFEM2D_edge_stiffness.m']);
    end
end
