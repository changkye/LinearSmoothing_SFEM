...
{
struct Geometry { double L; double H; };
struct LinearElasticMaterial { double young; double poisson; };
struct NeoHookeanMaterial { double mu; double lambda_like; };
...
inline LinearElasticMaterial linear_material_for(std::string_view problem_type)
{
    ...
    if (problem_type == "linear_patch") { material.poisson = 0.0; }
    return material;
}
